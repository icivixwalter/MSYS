...
'//@UTILITA@DIM@COLLETION_(LE COLLEZIONI DI OGGETTI DA CARICARE PER LA GESTIONE DELLE UTILITA)
  Public colTables_UTILITA As Collection    '//....vedi  @UTILITA@IMPORTA@COLLETION ...
  Public colQueryes_UTILITA As Collection
  Public colModules_UTILITA As Collection
...
Private Sub Cmd_Importa_MACRO_Click()
    ...
    Dim deleteMacrosCount_i As Integer
...
Private Sub Cmd_IMPORTA_OGGETTI_UTILITA_Click()
    ...
    For Each varItem In colQueryes_UTILITA
        ...
        If QueryExists Then
            ...
            deleteQueryesCount_i = deleteQueryesCount_i + 1
            deleteNameQueryes_s = deleteNameQueryes_s & qryName & vbCrLf
            ...
            importedNameQueryes_s = importedNameQueryes_s & qryName & vbCrLf
        Else
    ...
    For Each varItem In colForms_UTILITA
        ...
        If formExists Then
            ...
            importedFormsCount_i = importedFormsCount_i + 1
    ...
    For Each varItem In colMacros_UTILITA
        ...
        If macroExists Then
            ...
            importedMacrosCount_i = importedMacrosCount_i + 1
    ...
    MsgBox "Importazione completata con successo!" & vbCrLf & vbCrLf & _
           ...
           "Query importate: " & importedQueryesCount_i & vbCrLf & _
           "Moduli importati: " & importedModuliCount_i & vbCrLf & _
           "Forms importati: " & importedFormsCount_i & vbCrLf & _
...
Public Sub CaricaOggetti_UTILITA(ByRef colTables_UTILITA As Collection, _
                          ByRef colQueryes_UTILITA As Collection, _
                          ByRef colModules_UTILITA As Collection, _
    ...
    Set colTables_UTILITA = New Collection
    Set colQueryes_UTILITA = New Collection
    Set colModules_UTILITA = New Collection
...
Private Sub Cmd_DELETE_OGGETTI_UTILITA_Click()
    ...
    For Each varItem In colModules_UTILITA
...
'//IMPORTA GLI OGGETTI QUERY DEL DB ESTERNO  NEL DB CORRENTE *** INIZIO ***
'//----------------------------------------------------------------------------------------//
...
'//PARAMETRI_DA_CAMBIARE _
    ...
    '//I COMANDI BUTTON PER GESTIRE L'IMPORTAZIONI DELLE QUERY: _
        Cmd_Importa_QUERY_Click         = ESEGUE IL CONTROLLO E L'IMPORTAZIONE _
        Cmd_Importa_QUERY_HELP_Click    = HELP GUIDA PER IL COMANDO DI IMPORTAZIONE QUERY

    '//OPERAZIONI ESEGUITE PER LA LAVORAZIONE DELLA COLLECTION QUERY _
        @QUERY@INSERT.01=CHIAMO LA FUNZIONE CHE CREA LA COLLECTION QUERY E LA CARICA CON GLI OGGETTI SCELTI _
        @QUERY@INSERT.02=PRECARICO LA COLLECTION QUERY DAIMPORTARE _
        @QUERY@INSERT.03=CREA LA COLLECTION QUERY _
        @QUERY@INSERT.04=SELECT TRA VARI OGGETTI QUERY _
        @QUERY@INSERT.05=RESET CASELLA DI TESTO _
        @QUERY@INSERT.06=STAMPO TESTATA DI RIEPILOGO COLLEZIONI E VISUALIZZAZIONE PATH ED OGGETTI PRECARICATI _
        @QUERY@INSERT.07=INSERISCO LA PATH NELLA CASELLA DI TESTO ED AGGIORNO _
        @QUERY@INSERT.08=ITERO NEGLI OGGETTI QUERY CARICATI E FACCIO UN STAMPA DI CONTROLLO E CARICO LA STRINGA DA STAMPARE _
        @QUERY@INSERT.09=STAMPO GLI OGGETTI NELLA CASELLA DI TESTO E LA AGGIORNO _
        @QUERY@INSERT.10=RESTITUISCO LA COLLEZIONE AL CHIAMANTE _
        @QUERY@INSERT.11=ITERO NELLA COLLECTION QUERY PER UNA STAMPA DI CONTROLLO DI STAMPA
     '//@QUERY@INSERT.12=Messaggio finale di riepilogo SE ABILITATO CON TRUE - FALSE DISABILITATO _
        @QUERY@INSERT.13=RECUPERO LA PATH ED IL FILE.MDB PER LE IMPORTAZIONI DALLE CASELLE DI TESTO _
        @QUERY@INSERT.14=IF MESSAGGIO DI IMPORTAZIONE : TRUE = MESSAGGIO FALSO = NESSUN MESSAGGIO _
        @QUERY@INSERT.15=CONTROLLO SE PROGETTO INTERNO (NON CARICA LE QUERY) OPPURE ESTERNO (CARICA LE QUERY) _
        @QUERY@INSERT.17=INIZIO IMPORTAZIONE QUERY -> MA PRIMO CREO VARIABILI DB CORRENTE ED ESTERNO _
        @QUERY@INSERT.18=INIZIO IMPORTAZIONE QUERY -> APRO IL DB ESTERNO E CONFRONTO QUERY DB ESTERNO = QUERY COLLECTION _
        @QUERY@INSERT.19=INIZIO IMPORTAZIONE QUERY -> CHIAMO ROUTINE ESTERNA PER VERIFICARE ESISTENZA QUERY _
        @QUERY@INSERT.20=INIZIO IMPORTAZIONE QUERY -> ROUTINE ESTERNA VERIFICA SE QUERY DB ESTERNO ESISTE NELLA COLLETION QUERY DA CARICARE NEL  _DB CORRENTE _
        @QUERY@INSERT.21=INIZIO IMPORTAZIONE QUERY -> ROUTINE ESTERNA RESTITUISCE QUERY CHE GIA ESISTE NEL DB CORRENTE PRIMA CANCELLA POI IMPORTA (evito duplicati) _
        @QUERY@INSERT.22=INIZIO IMPORTAZIONE QUERY -> CONTEGGIO LE QUERY CANCELLATE _
        @QUERY@INSERT.23=INIZIO IMPORTAZIONE QUERY -> CONTEGGIO LE QUERY IMPORTATE _
        ...
        @QUERY@INSERT.24=CICLO FOR PER ANDARE ALL'ULTIMA QUERY CARICATA E FARE IL DoCmd.SelectObject  DELL'OGGETTO _
        @QUERY@INSERT.25=FINE,PRIMA DI USCIRE RILASCIO GLI OGGETTI
...
'//IMPORTA GLI OGGETTI QUERY DEL DB ESTERNO  NEL DB CORRENTE  *** FINE ***
'//----------------------------------------------------------------------------------------//
...
'//----------------------------------------------------------------------------------------//
'//NOTE: dal database di origine importa solo gli oggetti Moduli controllando prima se _
            nel db corrente di destinazione già esiste il Modulo per effettuare la cancellazione _
            prima dell'importazione.
...
'//@MODULI@INSERT.16= INIZIO IMPORTAZIONE MODULI
    ...
    ' Itera sugli oggetti Moduli nel database di origine usando MSysObjects
    For Each varItem In objectModules
        ...
        If moduleExists Then
            ' Se il modulo esiste, prima lo elimino se è presente nel database corrente
            On Error Resume Next
            Application.VBE.VBProjects(1).VBComponents.Remove Application.VBE.VBProjects(1).VBComponents(myNameModule_s)
...
'//______________________________________________________________________________
                ...
                Case "COLLECTION_MACROS"
                    ...
                    '//------------------------------------------------------------------------//
                        ...
                        For Each macroName_v In par_LaCollection
